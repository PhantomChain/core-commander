...
core_update ()
{
    ...
    if [[ "$remote_version" == "$local_version" ]]; then
        ...
        info "You already have the latest PhantomChain version that we support."
    else
        STATUS_CORE_UPDATE="Yes"

        read -p "An update is available for PhantomChain, do you want to install it? [Y/n] : " choice
        ...
        if [[ -z "$choice" || "$choice" =~ ^(yes|y|Y) ]]; then
            ...
            git branch -D "$CORE_BRANCH" | tee -a "$commander_log"
            git fetch
            ...
            check_for_modifications "packages/core/lib/config/${CORE_NETWORK}/peers.json" "${CORE_CONFIG}/peers.json"
